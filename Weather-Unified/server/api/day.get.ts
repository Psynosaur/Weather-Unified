...
export default defineEventHandler(async (): Promise<DatePageData> => {
  try {
    // Get today's date in YYYY-MM-DD format
    const today = new Date().toISOString().split('T')[0]

    // Fetch from WURequest API with authentication
    const response = await backendFetch<Observation[]>(
      `/api/observations/date/${today}`
    )
    ...
    const { cloudiness, standardDeviation }
      = calculateCloudiness(backendObservations)
...
function calculateCloudiness(observations: Observation[]): {
  cloudiness: string
  ...
  const sumOfSquares = solarValues.reduce(
